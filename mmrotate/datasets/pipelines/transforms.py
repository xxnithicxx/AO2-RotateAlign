...
class RNormalize(Normalize):
    ...
    def __init__(self, mean, std, to_rgb=True):
        super(RNormalize, self).__init__(mean=mean, std=std, to_rgb=to_rgb)
    ...
    def __call__(self, results):
        ...
        for key in ['gt_bboxes', 'gt_bboxes_ignore']:
            if key in results:
                bboxes = results[key]
                bboxes[:, :2] /= np.array([W, H], dtype=np.float32)
                bboxes[:, 2:4] /= np.array([W, H], dtype=np.float32)
                results[key] = bboxes
        return results

@ROTATED_PIPELINES.register_module()
class RToTensor(object):
    def __init__(self):
        super(RToTensor, self).__init__()
        
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_tensor = F.to_tensor(img.copy())
            results[key] = img_tensor
