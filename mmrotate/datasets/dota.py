...
class DOTADataset(CustomDataset):
    ...
    def _results2submission(self, id_list, dets_list, out_folder=None):
        ...
        if osp.exists(out_folder):
            raise ValueError(
                f"The out_folder should be a non-exist path, "
                f"but {out_folder} is existing"
            )
        os.makedirs(out_folder)

        files = [osp.join(out_folder, "Task1_" + cls + ".txt") for cls in self.CLASSES]
        ...
        with zipfile.ZipFile(
            osp.join(out_folder, target_name + ".zip"), "w", zipfile.ZIP_DEFLATED
        ) as t:
            for f in files:
